...
//                        return super.createSuccessfulAuthentication(request, user);
    ...
    protected void configure(HttpSecurity http) throws Exception {
        http
                ...
                .sessionManagement(
                        s->s//.sessionCreationPolicy(p-> SessionCreationPolicy.S)
                            .sessionFixation(sessionFixationConfigurer -> sessionFixationConfigurer.changeSessionId())
                            .maximumSessions(2) //한 명의 유저가 보유할 수 있는 세션의 최대 개수
                            .maxSessionsPreventsLogin(true) //max session이 발생했을 때 기존 세션을 만료할 건지, 새로운 세션을 만료할건지 지정 (default : false)
