...
public class SpELTest {
    ...
    Person person = Person.builder().name("홍길동").height(178).build(); //사람 객체/인스턴스 생성

    Horse nancy = Horse.builder().name("nancy").height(160).build(); //말 객체/인스턴스 생성
    ...
    @Test
    void test_2() {
        parser.parseExpression("name").setValue(person, "박지성");
        assertEquals("박지성", parser.parseExpression("name").getValue(person, String.class));
    }
